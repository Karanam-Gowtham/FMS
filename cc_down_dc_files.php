...
<nav class="navbar">
    <div class="nav-container">
        <div class="nav-items">
            ...
            <span id="sp">&nbsp; >> &nbsp;</span><span class="sid"><a href="cc_acd_year.php?dept=<?php echo "$dept" ?>&designation=<?php echo "$desg" ?>" class="home-icon"><?php echo urlencode($desg); ?></a></span>
...
<div class="container111">
    ...
    if (!empty($selected_file_type)) {
        $sql = "SELECT Username, file_name, acd_year, file_type, file_path FROM dc_up_files WHERE file_type = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $selected_file_type);
        $stmt->execute();
