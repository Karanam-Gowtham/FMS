<?php
/**
 * FMS Document View Page
 *
 * Shows a single document's full details, meta data, files, workflow status,
 * action history, and approval/rejection controls (if the user has permission).
 *
 * URL: pages/documents/view.php?id={doc_id}
 */
...
// Authorization: check if user can view this document
$can_view = false;
$user_id = (int)$auth['user_id'];

// Uploader can always view their own documents
...
if (!$can_view) {
    ...
    echo '<p>You do not have permission to view this document.</p>';
    exit;
}

// Load related data
$meta_data = doc_get_meta($conn, $doc_id, $document['type_key']);
$files = file_get_by_document($conn, $doc_id);
$action_history = wf_get_action_history($conn, $doc_id);
$meta_fields = meta_get_fields($document['type_key']);
...
$page_title = htmlspecialchars($document['title']);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="container">
    ...
    <div class="action-section">
        ...
        <form method="POST" action="<?= BASE_URL ?>/pages/documents/approve.php" id="actionForm">
    ...
    <div class="section">
        ...
        <ul class="files-list">
            ...
            <li>
                ...
                <a href="<?= BASE_URL ?>/pages/documents/download.php?id=<?= $f['file_id'] ?>" class="btn-download">Download</a>
            </li>
            <?php endforeach; ?>
        </ul>
    </div>
