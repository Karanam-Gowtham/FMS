<?php
/**
 * API: Save NBA Criterion 3
 *
 * Handles POST requests to save Criterion 3 data including:
 * - JSON payload (PDF file paths for 3.1, 3.2, 3.3, 3.4, 3.5)
 */
...
try {
    ...
    // Using criterion_number = 3
    $crit_stmt = $conn->prepare("SELECT id FROM nba_criteria_data WHERE submission_id = ? AND criterion_number = 3");
    $crit_stmt->bind_param("i", $sub_id);
    $crit_stmt->execute();
    
    if ($crit_stmt->get_result()->num_rows > 0) {
        $update = $conn->prepare("UPDATE nba_criteria_data SET data_json = ? WHERE submission_id = ? AND criterion_number = 3");
        $update->bind_param("si", $final_json, $sub_id);
        $update->execute();
        $update->close();
    } else {
        $insert = $conn->prepare("INSERT INTO nba_criteria_data (submission_id, criterion_number, data_json) VALUES (?, 3, ?)");
    ...
    echo json_encode([
        'status' => 'success',
        'message' => 'Criterion 3 data saved successfully.',
