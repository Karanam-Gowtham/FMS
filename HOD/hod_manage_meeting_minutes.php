...
<head>
    ...
    <style>
        ...
        .main-container { margin-top: 100px; padding: 2rem; max-width: 1200px; margin-left: auto; margin-right: auto; }
        .card { background: white; border-radius: 0.75rem; box-shadow: 0 4px 6px rgba(0,0,0,0.1); padding: 1.5rem; }
        h1 { font-size: 1.5rem; font-weight: 700; color: #111827; margin-bottom: 1.5rem; }
        
        table { width: 100%; border-collapse: collapse; margin-top: 1rem; }
        th { background: #f9fafb; padding: 0.75rem; text-align: left; font-size: 0.875rem; font-weight: 600; color: #4b5563; border-bottom: 1px solid #e5e7eb; }
        td { padding: 1rem 0.75rem; font-size: 0.875rem; border-bottom: 1px solid #e5e7eb; }
        ...
        .btn { padding: 0.5rem 1rem; border-radius: 0.375rem; font-size: 0.875rem; font-weight: 500; cursor: pointer; border: none; transition: all 0.2s; }
        .btn-view { background: #3b82f6; color: white; }
        .btn-accept { background: #10b981; color: white; }
        .btn-reject { background: #ef4444; color: white; }
        .btn-bulk { background: #4b5563; color: white; margin-bottom: 1rem; }
        
        .modal { display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); }
        .modal-content { background: white; margin: 15% auto; padding: 2rem; border-radius: 0.5rem; width: 400px; }
        textarea { width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; margin: 1rem 0; }
        
        .navbar { background: white; position: fixed; top: 0; width: 100%; z-index: 100; box-shadow: 0 1px 2px rgba(0,0,0,0.05); }
        .nav-container { max-width: 1200px; margin: 80px auto 0; padding: 1rem; display: flex; align-items: center; }
...
<body>
    ...
    <div class="main-container">
        <div class="card">
            ...
            <form method="POST">
                <button type="submit" name="bulk_action" value="download" class="btn btn-bulk">Download Selected</button>
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <?php
                        $sql = "SELECT * FROM dept_files WHERE dept = ? AND file_type = ? ORDER BY uploaded_at DESC";
                        $stmt = $conn->prepare($sql);
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            $details = "";
                            if ($row['semester']) $details .= "Sem: " . $row['semester'] . " ";
                            if ($row['study_year']) $details .= "Year: " . $row['study_year'] . " ";
                            if ($row['meeting_no']) $details .= "Mtg #: " . $row['meeting_no'];
