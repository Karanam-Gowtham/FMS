...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $submission_time = date('d-m-Y H:m:s');

    // Insert data into the database
    $stmt = $conn->prepare("INSERT INTO s_conference_tab (Username , acd_year, uploaded_by, branch, paper_title, from_date, to_date, organised_by, location, certificate_path, paper_type, paper_file_path, submission_time) VALUES (?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
...
<div class="cont1">
    <div class="container">
        ...
        <form id="conference-form" method="POST" enctype="multipart/form-data" action="">
            ...
            <div class="form-group">
                ...
                <div class="form-group">
                    <label for="paper-type1">Branch:</label>
                    <select id="paper-type1" name="Branch" required>
        ...
        <!-- Paper File Input (Initially Hidden) -->
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            // Select dropdown and file upload div
            const paperTypeSelect = document.getElementById('paper-type');
            const paperUploadDiv = document.getElementById('paper-upload-div');

            // Add event listener for dropdown changes
            paperTypeSelect.addEventListener('change', function () {
                // Show file input only for "Paper Publication"
                if (this.value === 'paper_publication') {
                    paperUploadDiv.style.display = 'block';
                } else {
                    paperUploadDiv.style.display = 'none';
                }
            });
        });
    </script>
