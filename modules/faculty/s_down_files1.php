...
if (isset($_POST['export_sjournal'])) {
   ...
   ob_start();// start new buffer for excel
    ...
    $sql_sjournal = "SELECT * FROM s_journal_tab WHERE username = ? and branch = ?";
    $stmt_sjournal = $conn->prepare($sql_sjournal);
    $stmt_sjournal->bind_param("ss", $username,$branch_select);
...
if (isset($_POST['export_sconference'])) {
    ...
    $sql_sconference = "SELECT * FROM s_conference_tab WHERE Username = ? and branch = ?";
    $stmt_sconference = $conn->prepare($sql_sconference);
    $stmt_sconference->bind_param("ss", $username,$branch_select);
...
if (isset($_POST['export_sbodies'])) {
    ...
    $sql_sbodies = "SELECT * FROM s_bodies WHERE Username = ? and Body = ? and branch = ?";
    $stmt_sbodies = $conn->prepare($sql_sbodies);
    $stmt_sbodies->bind_param("sss", $username,$bodies_sub_select,$branch_select);
...
if (isset($_POST['export_sevents'])) {
    ...
    $sql_sevents = "SELECT * FROM s_events WHERE username = ? and branch = ? and activity = ?";
    $stmt_sevents = $conn->prepare($sql_sevents);
    $stmt_sevents->bind_param("sss", $username,$branch_select,$main_select);
    ...
    <form method="POST" action="">
                ...
                <div>
                    ...
                    <select id="branch-select" name="branch_select">
                        ...
                        <option value="CSE-CS" <?= $branch_select == 'CSE-CS' ? 'selected' : '' ?>>CSE-CS</option>
                        <option value="CSE-AI&ML" <?= $branch_select == 'CSE-AI&ML' ? 'selected' : '' ?>>CSE-AI&ML</option>
                        <option value="CSE-AI&DS" <?= $branch_select == 'CSE-AI&DS' ? 'selected' : '' ?>>CSE-AI&DS</option>
                        ...
                        <option value="MatheMatics" <?= $branch_select == 'MatheMatics' ? 'selected' : '' ?>>MatheMatics</option>
                        <option value="Physics" <?= $branch_select == 'Physics' ? 'selected' : '' ?>>Physics</option>
                        <option value="Chemistry" <?= $branch_select == 'Chemistry' ? 'selected' : '' ?>>Chemistry</option>
            ...
            case 'Journals':
                ...
                if ($result_sjournal->num_rows > 0) {
                    ...
                    while ($row = $result_sjournal->fetch_assoc()) {
                        ...
                        echo "<tr>
                                <td><input type='checkbox' name='selected_files[]' value='" . $row["id"] . "' data-filepath='" . $paperPath . "'></td>
