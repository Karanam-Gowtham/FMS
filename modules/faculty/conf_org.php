...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['merged_pdf_data']) && !empty($_POST['merged_pdf_data'])) {
        ...
        $merged_file_name = time() . '_merged_conf_org.pdf';
    ...
    $sql = "INSERT INTO conf_org_tab (username, branch, title, mode, date_from, date_to, organised_by, location, year, brochure, fdp_schedule_invitation, attendance_forms, feedback_forms, fdp_report, photos_pdf, merged_file, submission_time, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)";

    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssssssssssssss", $username, $dept, $title, $mode, $date_from, $date_to, $organised_by, $location, $year, $brochure, $schedule, $attendance, $feedback, $report, $photos_pdf, $merged_file_path, $status);

    if ($stmt->execute()) {
        echo "<script>alert('Conference Organized Record added successfully!'); window.location.href='acd_year.php?dept=" . urlencode($dept) . "';</script>";
    } else {
...
<head>
    <meta charset="UTF-8">
    <title>Upload Conference Organized</title>
    <style>
        body {
...
<body>
        ...
        <div class="nav-container">
            <div class="nav-items">
                ...
                <span class="sp">&nbsp; >> &nbsp;</span><span class="main"><a href="#" class="main-a"
                        style="text-decoration: none;">Conference Organized</a></span>
            </div>
        </div>
    </nav>

    <div class="container11">
        <h2>Upload Conference Organized Details</h2>
        <form id="confOrgForm" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="merged_pdf_data" id="merged_pdf_data">
            <div class="form-grid">
                <div class="form-group full-width">
                    <label for="title">Title of Conference:</label>
                    <input type="text" id="title" name="title" required placeholder="Enter Conference Title">
                </div>
                ...
                <div class="form-group full-width">
                    ...
                    <input type="text" id="organised_by" name="organised_by" required placeholder="Institution/Organization Name">
                </div>
                ...
                <div class="form-group">
                    <label for="schedule">Conference Schedule & Invitation:</label>
                ...
                <div class="form-group">
                    <label for="report">Conference Report:</label>
                    <input type="file" id="report" name="report" accept=".pdf,.doc,.docx">
                </div>

                <div class="form-group full-width">
                    <label for="photos_pdf">Photos of Conference: 
                        <a href="../../assets/templates/conf_org_photos_template.pdf" target="_blank" style="font-size: 0.9em; color: #4ca1af; text-decoration: underline; margin-left: 10px;">(Download Template)</a>
                    </label>
    ...
    <script>
        document.getElementById('confOrgForm').addEventListener('submit', async function (e) {
            e.preventDefault();
            ...
            try {
                ...
                // Ordered inputs
                const inputNames = ['brochure', 'schedule', 'attendance', 'feedback', 'report', 'photos_pdf'];
