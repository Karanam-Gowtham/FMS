...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload'])) {
    ...
    foreach ($_FILES['files']['name'] as $key => $file_name) {
        ...
        if (move_uploaded_file($fileTmpPath, $filepath)) {
            $sql = "INSERT INTO files5_2_1 (username, faculty_name, academic_year, student_name, programme, employer, pay, uploaded_at, file_name, file_path) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssssssss", $username, $faculty_name, $academic_year, $student_name, $programme, $employer, $pay, $currentDateTime, $filename, $filepath);
...
<nav class="navbar">
    ...
    <div class="upload-container">
        ...
        <form action="" method="POST" enctype="multipart/form-data" >
            ...
            <label for="employer">Name of Employer (with contact details):</label>
            <input type="text" id="employer" name="employer" required>

            <label for="pay">Pay Package at the Time of Appointment:</label>
